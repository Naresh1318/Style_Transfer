...
def create_args():
    ...
    parser.add_argument("--epochs", "-e",
                        help="No of epochs to optimize", default=10, type=int)
    return parser
...
content_image_path = options.content_path
...
# Training
combined_img = np.random.uniform(0, 255, (1, height, width, 3)) - 128.

epochs = options.epochs

for i in bar(range(epochs)):
    combined_img, min_val, info = fmin_l_bfgs_b(evaluator.loss, combined_img.flatten(),
                                                fprime=evaluator.grads, maxfun=20)
    # Save image after each epoch
    save_com = remove_preprocess(combined_img)
    imsave('combined_image_{}.jpg'.format(i), Image.fromarray(save_com))
